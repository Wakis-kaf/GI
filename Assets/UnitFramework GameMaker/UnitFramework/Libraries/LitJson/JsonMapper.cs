...
namespace LitJson
{
    ...
    public class JsonMapper
    {
        ...
        private static void AddObjectMetadata(Type type)
        {
            ...
            foreach (FieldInfo f_info in type.GetFields(BindingFlags.Instance
                                                        | BindingFlags.Static
                                                        | BindingFlags.Public
                                                        | BindingFlags.NonPublic))
            {
                bool isJsonSerializer = false;
                if (f_info.IsPrivate || f_info.IsAssembly)
                {
                    var serializerAttributesList = f_info.GetCustomAttributes(typeof(JsonSerializer), true);
                    var serializerAttributes = serializerAttributesList as ICollection<Attribute>;
                    if (serializerAttributes.Count <= 0)
                    {
                        
                        continue;
                    }
                    isJsonSerializer = true;
                }
                PropertyMetadata p_data = new PropertyMetadata();
                p_data.Info = f_info;
                p_data.IsField = true;
                ...
                if(isJsonSerializer)
                    data.Properties.Add("__"+f_info.Name, p_data);
                else
                {
        ...
        private static void AddTypeProperties(Type type)
        {
            ...
            foreach (PropertyInfo p_info in type.GetProperties())
            {
                //Debug.Log("PropertyInfo"+p_info.Name);
                if (p_info.Name == "Item")
                    continue;
...
#else
                    var propertyName =
 entry.Key is string ? (entry.Key as string) : Convert.ToString(entry.Key, CultureInfo.InvariantCulture);
#endif
            ...
            foreach (PropertyMetadata p_data in props)
            {
                ...
                if (p_data.IsField)
                {
                    if (((FieldInfo) p_data.Info).IsPrivate)
                    {
                        writer.WritePropertyName("__"+p_data.Info.Name);
                    }
                    else
                    {
